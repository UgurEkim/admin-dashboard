...
import { measurementIcons } from "@/components/multimeter/measurement-config";
...
import { measurementCapability } from "@/lib/multimeter/adapter";
...
export default function MultimeterPage() {
  ...
  const capability = measurementCapability(
    meter.definition.capabilities,
    measurementType,
  );
  const ranges = capability?.ranges[measurementMode] ?? [];
  const temperature = meter.definition.capabilities.temperature;
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-col items-start gap-2 sm:items-end">
          {meter.adapters.length > 1 && (
            <label className="text-sm font-medium">
              Device model
              <select
                className={selectClass}
                value={meter.definition.id}
                disabled={connected || busy}
                onChange={(event) => meter.selectAdapter(event.target.value)}
              >
                {meter.adapters.map((adapter) => (
          ...
          <p role="status" className="text-xs text-muted-foreground">
            {meter.identity
              ? `${meter.identity.manufacturer} ${meter.identity.model} · ${meter.identity.firmware} · ${meter.identity.serial}`
              : `Connect your ${meter.definition.label} by USB`}
          </p>
        </div>
      </div>
      ...
      {!connected && (
        <p className="text-sm text-muted-foreground">
          Use Chrome or Edge. {meter.definition.connectionHint} Close other apps
          using the meter first.
        </p>
      )}
      ...
      <Card>
        ...
        <CardContent className="grid gap-6 lg:grid-cols-10">
          <div className="min-w-0 space-y-6 lg:col-span-7">
            <div className="rounded-xl border border-blue-200 bg-blue-100 p-5 text-center text-blue-950 sm:p-6 lg:flex lg:h-48 lg:flex-col lg:justify-center">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              {(
                ...
                <div
                  key={label}
                  className={`min-h-20 rounded-lg border p-2.5 ${label === "Minimum" ? "border-amber-300/60 bg-amber-50/40 dark:border-amber-700/50 dark:bg-amber-950/20" : label === "Average" ? "border-blue-300/60 bg-blue-50/40 dark:border-blue-700/50 dark:bg-blue-950/20" : "border-rose-300/60 bg-rose-50/40 dark:border-rose-700/50 dark:bg-rose-950/20"}`}
                >
                  <p className="text-sm text-muted-foreground">{label}</p>
                  <p
                    className={`mt-1 break-all font-mono text-lg font-medium ${label === "Minimum" ? "text-amber-700 dark:text-amber-300" : label === "Average" ? "text-blue-700 dark:text-blue-300" : "text-rose-700 dark:text-rose-300"}`}
                  >
            ...
            <div className="grid items-start gap-4 sm:grid-cols-3">
              {capability && capability.modes.length > 1 && (
                <div className="space-y-2">
                  <p className="text-sm font-medium">Current type</p>
                  <div className="flex h-10 w-full rounded-lg border bg-muted/30 p-1">
                    {capability.modes.map((mode) => (
                      <Button
                        ...
                        className="h-full flex-1 font-mono text-sm font-semibold"
                      >
                        {mode}
                      </Button>
                    ))}
              ...
              {(ranges.length > 0 || capability?.autoRange) && (
                <label className="text-sm font-medium">
                  Measurement range
                  <select
                    className={selectClass}
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                <Eraser className="size-4 text-slate-600 dark:text-slate-300" />
                Clear graph
              </Button>
              <Button
                type="button"
          ...
          <div className="flex min-w-0 flex-col justify-between gap-6 lg:col-span-3">
            <div className="space-y-3">
              <p className="text-sm font-medium">Measurement</p>
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-3">
                {meter.definition.capabilities.measurements.map(({ type }) => {
                  ...
                  return (
                    <Button
                      ...
                      className={`h-16 min-w-0 flex-col gap-1 px-1 text-[0.7rem] font-semibold sm:h-20 sm:text-xs ${measurementType === type ? "" : "text-sky-700 dark:text-sky-300"}`}
                    >
            ...
            <p className="text-xs text-muted-foreground">
              Readings start automatically after connecting or changing the
              measurement. Use Stop to pause them. The read interval is the
              pause between completed readings.
            </p>
          </div>
        </CardContent>
      </Card>
